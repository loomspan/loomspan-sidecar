...
SC1 is implemented and locally verified at Sidecar `e3c3f58`; see the
[scaffold ticket](../tickets/2026-09-13-sidecar-scaffold.md) for closeout evidence.
Hosted CI remains deferred until framework publication.
...
- Initial framework source baseline: `385729a254261de128df491505acd8898cc0a021`.
  Local development consumes the already-installed Maven snapshot. The developer
  installs after framework changes, keeping the artifact and local source aligned;
  no separate artifact/source-revision verification is required for SC1. Sidecar builds
...
- Ordinary beta 4 development uses the locally installed framework snapshot.
  The developer re-runs framework `mvn install` after framework changes; rerun
  affected Sidecar checks. No snapshot repository is introduced. After local SC5
...
- [x] Using the developer-installed framework snapshot,
  `mvn verify` passes on clean committed Sidecar source using the snapshot dependency.
- [x] ArchUnit test fails on an internal import (proved by a deliberately
  failing example, then removed).
- [x] A YAML skill in the mounted directory registers at startup with its
  required model/connection configuration. Mounting a planner manifest alone
  does not configure its model.
- [x] Default scanning includes skills under `/sidecar/skills/` and excludes
  `/sidecar/rest-routes.yaml`; custom skill locations remain supported.
- [x] Push/PR CI is prepared to consume the published framework dependency.
